...
#define GTUIExtensionLog(colors, eFormat, format, ...) \
NSLog((uXCODE_COLORS_ESCAPE colors eFormat format uXCODE_COLORS_RESET), __PRETTY_FUNCTION__, __LINE__, ##__VA_ARGS__);
#define GTUIExtensionLog2(colors, eFormat, format, ...) \
NSLog((uXCODE_COLORS_ESCAPE colors eFormat format uXCODE_COLORS_RESET), ##__VA_ARGS__);
#define GTUIExtensionLog3(colors, eFormat, format, ...) \
NSLog((uXCODE_COLORS_ESCAPE colors eFormat uXCODE_COLORS_RESET), __PRETTY_FUNCTION__, __LINE__, #format, format, ##__VA_ARGS__);

#define GTUILog(format, ...) GTUIExtensionLog(@"fg0,178,238;", @"\n%s\n<%d> ", format, ##__VA_ARGS__)
#define GTUILogInfo(format, ...) GTUIExtensionLog2(@"fg0,168,0;", @"\n-> ", format, ##__VA_ARGS__)
#define GTUILogError(format, ...) GTUIExtensionLog2(@"fg255,41,105;", @"\n## ", format, ##__VA_ARGS__)

#define GTUILogObj(obj) GTUIExtensionLog3(@"fg89,89,207;", @"\n%s\n<%d> %s ⤭ %@ ⤪[;", obj)
#define GTUILogInteger(integer) GTUIExtensionLog3(@"fg89,89,207;", @"\n%s\n<%d> %s ⤭ %zd ⤪[;", integer)
#define GTUILogFloat(float) GTUIExtensionLog3(@"fg89,89,207;", @"\n%s\n<%d> %s ⤭ %lf ⤪[;", float)
#define GTUILogRect(rect) GTUIExtensionLog3(@"fg89,89,207;", @"\n%s\n<%d> %s ⤭ %@ ⤪", NSStringFromCGRect(rect))
#define GTUILogSize(size) GTUIExtensionLog3(@"fg89,89,207;", @"\n%s\n<%d> %s ⤭ %@ ⤪[;", NSStringFromCGSize(size))
#define GTUILogPoint(point) GTUIExtensionLog3(@"fg89,89,207;", @"\n%s\n<%d> %s ⤭ %@ ⤪[;", NSStringFromCGPoint(point))
#define GTUILogEdgeInsets(edgeInsets) GTUIExtensionLog3(@"fg89,89,207;", @"\n%s\n<%d> %s ⤭ %@ ⤪[;", NSStringFromUIEdgeInsets(edgeInsets))
#define GTUILogLine GTUIExtensionLog(@"fg217,56,41;", @"\n%s\n<%d> ----- ----- ----- -----",)

#define GTUIAssert(condition, format, ...) \
do { \
_Pragma("clang diagnostic push") \
_Pragma("clang diagnostic ignored \"-Wformat-extra-args\"") \
if ((condition) == NO) \
GTUILogError(format, ##__VA_ARGS__); \
_Pragma("clang diagnostic pop") \
} while (0);

#define GTUIAssertParamNotNil(param, format, ...) \
GTUIAssert(kNilOrNull(param) == NO, format, ##__VA_ARGS__)
...
#define kAboveIOS12 ([[[UIDevice currentDevice] systemVersion] floatValue] >= 12.0)
...
#define kRetina ([[UIScreen mainScreen] scale] >= 2.0)
...
#define kIPhone4                (kIPhone && kScreenMaxLength  < 568.0)
#define kIPhone5                (kIPhone && kScreenMaxLength == 568.0)
#define kIPhone6                (kIPhone && kScreenMaxLength == 667.0)
#define kIPhone6P               (kIPhone && kScreenMaxLength == 736.0)
#define kIPhoneX                (kIPhone && kScreenMaxLength == 812.0)
#define kIPhoneX_Max            (kIPhone && kScreenMaxLength == 896.0)
